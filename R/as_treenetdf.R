...
as_treenetdf.list <- function(data, children_name = "children",
  ...
  makelistofdfs <- function(data) {
    ...
    c(
      list(data[names(data)[!names(data) %in% children_name]]),
...
as_treenetdf.phylo <- function(data, ...) {
  ...
  df <- rbind(c(
    nodeId = rootId, parentId = NA, name = NA, edge.length = 0,
    depth = 0, height = max(df$depth)
  ), df)
...
as_treenetdf.igraph <- function(data, root = "root", ...) {
  ...
  if (length(rootId) > 1) {
    rootdf <- Reduce(function(x, y) {
      rbind(x, c(
        nodeId = y, parentId = root,
        setNames(rep(NA, length(names(df)) - 2), names(df)[-(1:2)])
      ))
    }, rootId, c(
      nodeId = root, parentId = NA,
      setNames(
        rep(NA, length(names(df)) - 2),
        names(df)[-(1:2)]
      )
    ))
...
as_treenetdf.data.frame <- function(data,
  ...
  if (df_type == "treenetdf") {
    ...
    names(data)[namestoswitch] <- names(cols)[match(
      names(data)[namestoswitch],
      cols
    )]
    ...
    return(data.frame(
      nodeId = nodeId, parentId = parentId, name = name,
      stringsAsFactors = F
    ))
  }
}
